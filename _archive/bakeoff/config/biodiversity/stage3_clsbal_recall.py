"""
Stage 3 — clsbal sampler + RECALL LOSS (Semi-nat claw-back lever, docs/MINORITY_STRATEGY §16.4/§16.5).

Clone of stage3_clsbal.py changing ONLY the loss: the unweighted SoftCE term is swapped for the
recall-weighted CE (Tian et al. 2022, "Striking the Right Balance", ICRA; geoseg/losses/recall_ce.py),
which adds a per-class weight = (1 - recall_c), up-weighting the low-recall minorities (Settlement,
Semi-natural). Same clsbal sampler, same Dice term, same init/optimiser/epochs. Isolates the recall
loss's effect on the minorities. Compare to plain clsbal (seed42: Settlement 73.85, Semi-nat 73.16).
Watch IoU (not just recall) — pushing recall can over-predict minorities and dent precision.

Run:
  SEED=42 PYTHONPATH=. python -m train.train_supervision -c config/biodiversity/stage3_clsbal_recall.py --force
"""
...
# ====================== Logging / checkpoints ======================
weights_name = "stage3_clsbal_recall"
