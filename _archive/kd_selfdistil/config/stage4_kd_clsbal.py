"""
Stage 4 KD — clsbal SAMPLER VARIANT (defensibility gate, docs/MINORITY_STRATEGY §16.2).

BYTE-IDENTICAL to stage4_kd.py except (a) inits from the clsbal Stage-3 ckpt, (b) uses the clsbal
sampler TSV, (c) isolated weights_name. Same OEM teacher, same KD (alpha=0.10, T=2.0), same optim/
schedule/epochs. Purpose: test whether the standard class-balanced sampler matches A0 AFTER distillation
(A0+KD seed42 = Semi-nat 79.76). If it ties, the bespoke hardness×richness sampler can be dropped for a
simpler, more defensible paper. Compare to stage4_kd seed42 (the A0 baseline).
...
# Initialise student from the clsbal Stage 3 checkpoint (the bake-off arm, on Sonic $BASE/bakeoff/...)
pretrained_ckpt_path = (
    "model_weights/biodiversity/stage3_clsbal/"
    "stage3_clsbal.ckpt"
)
...
if not weights_path_tsv.exists():
    raise FileNotFoundError(
        ...
        "Generate them with: python scripts/data_prep/build_clsbal_sampler.py"
    )
