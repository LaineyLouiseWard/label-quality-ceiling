"""
KD MAPPING-TEST ARM **A** (targeted grounding).

Identical to config/biodiversity/stage5_norep.py EXCEPT the KD mapping matrix: the three
demonstrably-mismatched OEM rows (Bareland, Water, Agriculture) are replaced with the measured
teacher->GT soft confusion; the clean/grounded rows (incl. Rangeland 0.73/0.27) are kept.
...
Run (after the confusion matrix exists at artifacts/teacher_oem_gt_confusion.npz):
  PYTHONPATH=. python -m train.train_kd -c config/biodiversity/stage5_mapA.py
"""
...
# --- THE ONLY METHODOLOGICAL CHANGE: data-grounded mapping (Option A, targeted) ---
mapping_matrix = build_mapping_from_confusion("A", alpha=rangeland_split_alpha)
...
print(f"[Stage5-mapA KD] Loaded weights for {len(id_to_weight)} ids. "
      f"Missing={missing}/{len(train_dataset.img_ids)}")
if missing > 0:
    raise RuntimeError(
        f"[Stage5-mapA KD] {missing}/{len(train_dataset.img_ids)} train ids have no sampling weight."
    )
